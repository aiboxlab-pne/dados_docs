...
def break_disciplinas(data: pd.DataFrame, list_disciplina:list) -> pd.DataFrame:
    """
    Desagrega o DataFrame com base nas disciplinas selecionadas desde que
    contenham valores 1.
    A função retorna um DataFrame concatenado de todos os subconjuntos criados.

    Parâmetros:
    - data (pd.DataFrame): O DataFrame original que contém os dados.
    - list_disciplina (list): Uma lista de strings representando os nomes das
    disciplinas.

    Retorna:
    - pd.DataFrame: Um DataFrame concatenado de todos os subconjuntos criados,
                    onde cada subconjunto contém apenas as linhas onde a
                    disciplina correspondente é igual a 1.
    """
...
def process_ensino_infantil(df_dim_docentes, df_formacao_nome, year):
    ...
    return (df_dim_docentes
        ...
        .assign(FORMACAO_COD=lambda x:x.apply( lambda row:'|'.join(sorted(
            ...
            if y != 'None'])), axis=1))
        .assign(FORMACAO_NOME_1=lambda x:
            x[['CO_CURSO_1']].merge(df_formacao_nome,
             left_on='CO_CURSO_1',right_index=True,how='left')['FORMACAO_NOME']
            .where(x['TP_SITUACAO_CURSO_1'].isin([1]),np.nan))
        .assign(FORMACAO_NOME_2=lambda x:
            x[['CO_CURSO_2']].merge(df_formacao_nome,
             left_on='CO_CURSO_2',right_index=True,how='left')['FORMACAO_NOME']
            .where(x['TP_SITUACAO_CURSO_2'].isin([1]),np.nan))
        .assign(FORMACAO_NOME_3=lambda x:
            x[["CO_CURSO_3"]].merge(df_formacao_nome,
            left_on="CO_CURSO_3", right_index=True, how="left")["FORMACAO_NOME"]
            .where(x["TP_SITUACAO_CURSO_3"].isin([1]), np.nan))
        .assign(FORMACAO_NOME=lambda x:x.apply( lambda row:'|'.join(sorted(
            [y for y in [str(row['FORMACAO_NOME_1']),
                         str(row['FORMACAO_NOME_2']),
                         str(row['FORMACAO_NOME_3'])]
            if y != 'nan'])), axis=1))
...
def process_fund_anos_iniciais(df_dim_docentes, df_formacao_nome, year):
    return (df_dim_docentes
        ...
        .assign(FORMACAO_ADEQUADA_ESPECIFICA=lambda x: process_formacao_adequada(x, year))
        .assign(DISCIPLINA_NOME='MULTIDISCIPLINAR')
        .assign(FORMACAO_ADEQUADA_PEDAGOGIA=lambda x: process_formacao_adequada(x, year))
        .assign(DISCIPLINA_NOME=lambda x: x['DISCIPLINA_NOME_BKP'])
        .assign(EXCECAO=lambda x: x['DISCIPLINA_NOME'].isin(['IN_DISC_LINGUA_ESTRANGEIRA']))
        .assign(FORMACAO_ADEQUADA=lambda x:
                (x['EXCECAO'] & x['FORMACAO_ADEQUADA_ESPECIFICA'])
                | (~x['EXCECAO'] & (x['FORMACAO_ADEQUADA_PEDAGOGIA'] | x['FORMACAO_ADEQUADA_ESPECIFICA'])))
...
def model(dbt, fal):
    ...
    year = 2014
    # load
    df_dim_docentes = dbt.source("raw", "DIM_DOCENTES_CO_2014")
...
def source(*args, dbt_load_df_function):
    sources = {"raw.DIM_DOCENTES_CO_2014": "\"postgres\".\"raw\".\"DIM_DOCENTES_CO_2014\""}
    key = '.'.join(args)
...
class this:
    ...
    database = "postgres"
    schema = "dbt_staging"
    identifier = "dim_educ_bas_docentes_15_2014_co"
    
    def __repr__(self):
        return '"postgres"."dbt_staging"."dim_educ_bas_docentes_15_2014_co"'
