...
def ref(*args, **kwargs):
    refs = {"dim_ideb": "\"postgres\".\"dbt_staging\".\"dim_ideb\"", "info_municipios": "\"postgres\".\"dbt_staging\".\"info_municipios\""}
    key = '.'.join(args)
...
class this:
    ...
    database = "postgres"
    schema = "dbt_serving"
    identifier = "fct_meta_sete"
    
    def __repr__(self):
        return '"postgres"."dbt_serving"."fct_meta_sete"'
...
def main(read_df, write_df, fal_context=None):
  ...
  return write_df(
      'postgres.dbt_serving.fct_meta_sete',
      df
  )