...
def model(dbt, fal):
    ...
    year = 2016
    # load
    df_dim_docentes = dbt.source("raw", "DIM_DOCENTES_CO_2016")
...
def source(*args, dbt_load_df_function):
    sources = {"raw.DIM_DOCENTES_CO_2016": "\"postgres\".\"raw\".\"DIM_DOCENTES_CO_2016\""}
    key = '.'.join(args)
...
class this:
    ...
    database = "postgres"
    schema = "dbt_staging"
    identifier = "dim_educ_bas_docentes_15_2016_co"
    
    def __repr__(self):
        return '"postgres"."dbt_staging"."dim_educ_bas_docentes_15_2016_co"'
...
def main(read_df, write_df, fal_context=None):
  ...
  return write_df(
      'postgres.dbt_staging.dim_educ_bas_docentes_15_2016_co',
      df
  )